...
function setup() {
  ...
  for (let i = 0; i < fishCount; i++) {
    fish.push({
      x: random(width),
      y: random(height),
      xSpeed: random(0.5, 1), // Slower speed
      ySpeed: random(0.5, 1), // Slower speed
...
function draw() {
  ...
  ///make sure you add the bubbles

  
  drawUnderwaterPlant(width / 2, height - 50);
  drawUnderwaterPlant(width -100, height - 50);
  drawUnderwaterPlant(width -200, height - 50);
  drawUnderwaterPlant(width -300, height - 50);
  drawUnderwaterPlant(width -500, height - 50);
  drawUnderwaterPlant(width -600, height - 50);
  drawUnderwaterPlant(width -700, height - 50);
  
  
  drawCreature(creatureX, creatureY);

  // Check keyboard input for creature movement
